...
export function DonadorDashboard() {
  ...
  // Navegación
  const irA = (v) => {
    // Si es "donate" redirige a acopio (donde está el centro de acopio)
    if (v === "donate") setView("acopio");
    else setView(v);
  };
        ...
        <div className="flex items-center justify-between px-6 py-4 max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-4">
            <div className="hidden md:flex gap-6 mr-6">
              <button onClick={() => irA("inicio")} className={`font-medium transition-colors ${view === "inicio" ? "text-indigo-600" : "text-slate-500 hover:text-indigo-600"}`}>
                Inicio
              </button>
              <button onClick={() => irA("donate")} className={`font-medium transition-colors ${view === "acopio" ? "text-indigo-600" : "text-slate-500 hover:text-indigo-600"}`}>
                Donar
              </button>
              <button onClick={() => irA("impacto")} className={`font-medium transition-colors ${view === "impacto" ? "text-indigo-600" : "text-slate-500 hover:text-indigo-600"}`}>
                Impacto
              </button>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-5 md:px-10 mt-8 space-y-12">
          ...
          <>
            ...
            <section className="space-y-6">
              <div className="flex items-center justify-between">
                <h2 className="text-h2 font-h2 text-on-background">Impacto de por vida</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div className="md:col-span-2 bg-white p-8 rounded-3xl shadow-sm border border-indigo-50 flex flex-col justify-between group hover:shadow-md transition-shadow relative overflow-hidden">
                  ...
                  <div className="relative">
                    ...
                    <h3 className="text-body-lg text-body-lg text-on-surface-variant">Comidas donadas</h3>
                  ...
                  <div className="mt-8 pt-6 border-t border-indigo-50/50 flex items-center gap-2 text-primary">
                    ...
                    <span className="font-label-sm text-label-sm">+12% vs el mes pasado</span>
                  </div>
                </div>

                {/* Medium Card 1 - Personas */}
                <div className="bg-surface-container-lowest p-8 rounded-3xl shadow-sm border border-indigo-50 flex flex-col justify-between hover:shadow-md transition-shadow">
                  <div>
                    ...
                    <h3 className="text-body-md text-body-md text-on-surface-variant">Personas ayudadas</h3>
                    <p className="text-[40px] font-bold text-on-background mt-1">3,204</p>
                  </div>
                  <p className="font-label-sm text-label-sm text-outline mt-4">Alcance comunitario directo</p>
                </div>
                ...
                <div className="bg-surface-container-lowest p-8 rounded-3xl shadow-sm border border-indigo-50 flex flex-col justify-between hover:shadow-md transition-shadow">
                  <div>
                    ...
                    <h3 className="text-body-md text-body-md text-on-surface-variant">CO₂ reducido</h3>
                    <p className="text-[40px] font-bold text-on-background mt-1">4.2<span className="text-2xl font-semibold">t</span></p>
                  </div>
                  <p className="font-label-sm text-label-sm text-outline mt-4">Equivalente a 84 árboles</p>
                </div>
              </div>
            </section>
            ...
            <section className="space-y-6">
              <div className="flex items-center justify-between">
                <h2 className="text-h2 font-h2 text-on-background">Donaciones recientes</h2>
              ...
              <div className="space-y-4">
                {[
                  { nombre: "Fresh Harvest Bundle", fecha: "Nov 14, 2023", lugar: "Community Food Bank", comidas: 42, estado: "Distributed", img: "fresh-produce" },
                  { nombre: "Artisan Bakery Surplus", fecha: "Nov 10, 2023", lugar: "St. Jude's Shelter", comidas: 115, estado: "Distributed", img: "bakery" },
                  { nombre: "Dairy Essentials Pack", fecha: "Nov 03, 2023", lugar: "North Bay Outreach", comidas: 28, estado: "In Transit", img: "dairy" },
                ].map((item, i) => (
                  <div key={i} className="group flex flex-col md:flex-row md:items-center justify-between p-6 bg-white rounded-2xl border border-indigo-50 hover:border-primary-container/30 transition-all hover:shadow-md gap-4">
                    <div className="flex items-center gap-5">
                      ...
                      <div>
                        <h4 className="text-h3 font-h3 text-on-background">{item.nombre}</h4>
                        <p className="font-label-sm text-label-sm text-outline">{item.fecha} • {item.lugar}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-8 md:gap-16">
                      <div className="text-left md:text-right">
                        <p className="font-label-sm text-label-sm text-secondary uppercase tracking-tight">Impacto</p>
                        <p className="text-body-lg font-body-lg font-bold text-on-background">{item.comidas} Comidas</p>
                      </div>
                      <div className="text-left md:text-right">
                        <p className="font-label-sm text-label-sm text-secondary uppercase tracking-tight">Estado</p>
                        <span className={`px-3 py-1 rounded-full font-label-sm text-label-sm inline-flex items-center gap-1 ${
                          item.estado === "Distributed"
                            ? "bg-emerald-50 text-emerald-700"
                            : "bg-indigo-50 text-indigo-700"
                        }`}>
                          <span className="material-symbols-outlined text-sm" style={{ fontVariationSettings: "'FILL' 1" }}>
                            {item.estado === "Distributed" ? "check_circle" : "local_shipping"}
                          </span>
                          {item.estado}
                        </span>
                      </div>
              ...
              <button className="w-full py-4 rounded-2xl border-2 border-dashed border-indigo-100 text-primary font-label-sm text-label-sm hover:bg-indigo-50/50 transition-colors">
                Ver todo el historial de donaciones
              </button>
            </section>
          </>
        )}
        ...
        {view === "impacto" && (
          <section className="text-center py-16">
            ...
            <h1 className="text-h2 font-h2 text-on-background">Impacto</h1>
            <p className="text-body-md text-on-surface-variant mt-2">
              Próximamente: reportes detallados de impacto
            </p>
          </section>
        )}
      </main>
      ...
      <nav className="fixed bottom-0 left-0 w-full z-50 bg-white/90 backdrop-blur-xl border-t border-indigo-50 shadow-[0_-8px_30px_rgba(79,70,229,0.08)] rounded-t-3xl md:hidden">
        <div className="flex justify-around items-center px-4 pt-3 pb-6">
          {[
            ...
            { key: "donate", icon: "volunteer_activism", label: "Donar" },
            { key: "impacto", icon: "analytics", label: "Impacto" },
            { key: "perfil", icon: "person", label: "Perfil" },
          ].map((item) => {
            const activo = view === item.key || (item.key === "donate" && view === "acopio");
            return (
              <button
                key={item.key}
              ...
              >
                <span className="material-symbols-outlined mb-1" style={activo ? { fontVariationSettings: "'FILL' 1" } : {}}>
                  {item.icon}
                </span>
